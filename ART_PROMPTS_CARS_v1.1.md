...
3. **Transparency.** Ask for a real alpha channel first — the UI does it now,
   and mostly gets it right. Magenta stays in every prompt as the named
   fallback, so a generation that fails transparency still lands on a colour I
   can key rather than on a white box or a scene. Both paths are handled on my
   end; the QA pass records which one each file actually arrived on. If a
   transparent one comes back with pink fringing, the model painted magenta
   and then cut it out badly — regenerate rather than send it. If it comes back
   as a drawn grey checkerboard, that is an opaque image of transparency, not
   transparency; regenerate that too.
...
One single object, centered, filling most of the frame, isolated on a fully TRANSPARENT background — a real alpha channel, never a drawn checkerboard and never a scene. If transparency is unavailable, fall back to a FLAT SOLID MAGENTA #FF00FF field with no texture, gradient or shadow. Either way keep magenta and pink out of the object itself. Neutral overhead lighting with a soft contact shadow directly beneath the object only, never a long directional shadow. The asset must still read clearly at 10% size against a dark floor. Palette anchors: floor #303b46, wall #46545f, brick #845345, wood #80623a, metal #788792, foliage #3b5238, accent #e8b53a.
...
An overlay decal seen from directly above: a loose scatter of about eight bullet holes punched through painted sheet metal, each a small dark puncture with a bright torn bare-metal rim and a faint dent ring. Spread irregularly with plenty of empty background between them. There must be NO background panel, NO car and NO surface behind the holes — only the holes themselves floating on the empty background, because this is stamped as a transparent overlay onto vehicle panels.
...
An overlay decal seen from directly above: a dense cluster of about twenty-five bullet holes through painted sheet metal, tightly grouped as if from a burst, each a dark puncture with a bright torn bare-metal rim, some overlapping into ragged tears, with buckling and dishing of the metal around the densest part. NO background panel, NO car and NO surface behind them — only the holes on the empty background.
...
An overlay decal seen from directly above: a spray of shattered safety-glass fragments lying on the ground, small pale blue-white cubes and chips catching the light, densest at one edge and thinning outward. NO ground surface and NO background behind them — only the fragments on the empty background.
...
- v1.1 (2026-08-15): Transparency flipped to alpha-first. GPT does real transparency now and mostly gets it right, so every prompt asks for an alpha channel and names magenta as the fallback rather than demanding magenta outright. Both paths were already handled on the processing side — batch 1 arrived mixed, one native and three keyed — so this changes what is asked for, not what can be accepted. The four damage overlays stop describing the backdrop as magenta, since a decal of bullet holes is mostly edge and keying costs it the most.
